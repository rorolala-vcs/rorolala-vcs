//! The exit code of the last run, kept in Rorolala's own file for the user.
//!
//! It is what `rola explain exit-code` explains when it is given no number: the run just
//! before is the one being asked about, and all of it that is kept is a number. It is the
//! smallest of the files Rorolala keeps — written once as a run ends, read once when it is
//! asked about — and a machine that does not name a local data directory has nowhere to
//! keep it, so both sides stay silent about a file they cannot reach.
...
///
/// This runs as the run ends, so a failure is silent: there is nothing left to report it
/// to, and a record that was not kept only means the next `rola explain exit-code` has
/// nothing to explain.
...
/// The exit code the run before this one ended with, if one was recorded.
///
