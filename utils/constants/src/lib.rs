...
mod tests {
    ...
    #[test]
    fn the_workspace_layout_names_its_own_paths() {
        assert_eq!(WORKSPACE_DATA_DIR, "./.rola/");
        assert_eq!(WORKSPACE_CONFIG_PATH, "./.rola/workspace.toml");
        assert_eq!(WORKSPACE_KEYS_DIR, "./.rola/auth/");
    }
    ...
    #[test]
    fn the_key_scopes_and_names_are_pinned() {
        assert_eq!(GLOBAL_KEYS_DIR, ".rola/keys");
        assert_eq!(USER_KEYS_DIR, "rola/keys");
        assert_eq!(ENV_KEYS_DIR, "keys");
        assert_eq!(HOME_ENV_VAR, "ROLA_HOME");
    }

    #[test]
    fn the_key_extensions_are_pinned() {
        assert_eq!(PUBLIC_KEY_EXTENSION, "pub");
        assert_eq!(PRIVATE_KEY_EXTENSION, "pem");
    }
}
